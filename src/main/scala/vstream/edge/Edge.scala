...
case class ConnectedSingleOutputEdge(targetNode: InputNode) extends SingleOutputEdge {
  override def send(payload: Payload): Unit = targetNode.receive(payload)
}
case object UnconnectedSingleOutputEdge extends SingleOutputEdge {
  override def send(payload: Payload): Unit = sys.error("can't send payload to downstream, edge is not connected.")
